...
export default {
  ...
  */
    ...
    function initLines(lat) {
      ...
      var points = [];
      points.push(
        new THREE.Vector3(longitude2world(-180), 0, latitude2world(lat))
      );
      points.push(
        new THREE.Vector3(longitude2world(180), 0, latitude2world(lat))
      );
    ...
    controls.addEventListener("change", function (e) {
      ...
      if (
        ...
        if (
          Math.abs(cameraRotation.x - e.target.object.rotation.x) <
            NUMBER_MIN &&
          Math.abs(cameraRotation.y - e.target.object.rotation.y) <
            NUMBER_MIN &&
          Math.abs(cameraRotation.z - e.target.object.rotation.z) < NUMBER_MIN
        ) {
          mode = "zoom";
    ...
    function dq() {
      ...
      if (
        ...
        map
          .getView()
          .setCenter(fromLonLat([centerLongitude, mapCenterLonLat[1]]));
